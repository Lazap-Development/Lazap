<script>
import { getInstalledGames, loadGames } from '@/components/launchers/find-games';
import { updateSetting } from '@/components/utils';
...
window.addEventListener("load", function () {
  ...
  /*window.onload = */async function xd() {
    ...
    setTimeout(() => loadGames('recentGamesListMainPage'), 200);
    ...
    document.getElementById('home-btn').addEventListener('click', async function() {
      ...
      await loadGames('recentGamesListMainPage');
    });
    ...
    document.querySelectorAll('input[id^=setting-]').forEach((input) => {
      input.addEventListener('change', () => {
        updateSetting('updateSetting', input.id.split('-')[1], document.querySelector(`input[id=${input.id}]`).checked);
      });
    });
  }
  document.getElementById('main-loading-overlay').style.opacity = '0';
