  ...
  if (type === "allGamesList") {
    ...
    for (let i = 0; i < games.length; i++) {
      const game =
        ...
        ) ?? (await getGames(games[i].GameID, games[i].LauncherName));
      if (
...
async function loadGames(id, data, stored) {
  ...
  (await filterAndSort(games, id, list, stored))
    .reverse()
    .forEach(async (game, i, arr) =>
      Elements.createGameElement(game, id, list, arr[i - 1])
    );
  ...
  if (!data) {
    loadGames(id, [
      ...games,
      ...(await getInstalledGames(["XboxGames.js"])),
    ]).then((d) =>
...
async function createProcess_windows(
  ...
  const instance = invoke("launch_game", { exec: Command, args: "" }).then(
    () => {
      VisibilityState({ LauncherName, DisplayName });
...
async function createProcess_linux(
  ...
  const instance = invoke("launch_game", { exec: Command, args: Args }).then(
    () => {
      VisibilityState({ LauncherName, DisplayName });
      processes.delete(GameID);
    }
...
export { getInstalledGames, loadGames, getGames, Elements };
...
class Elements {
  ...
  static async getStarElement(game) {
    ...
    starIcon.addEventListener("click", async () => {
      const solidOrEmpty = await toggleFavourite(
        game.GameID,
        game.LauncherName
      );
      starIcon.style.filter =
  ...
  static async createGameElement(game, id, list, prev) {
    if (game.LauncherName === "CustomGame" && !prev) {
      ...
      prev.push(game);
      const filtered = (
        await filterAndSort(prev, "allGamesList", list)
      ).reverse();
      prev =
        filtered[
