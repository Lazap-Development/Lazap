...
async function getUplayLocation(checkForUplay = true) {
  ...
  if ((await os.platform()) === "win32") {
    ...
    if (stderr) return;
      ...
      launcher_location = registry_res
        .split(window.__TAURI__.os.EOL.repeat(2))[0]
        .split("REG_SZ")[1]
        .split("\r\n\r\n")[0]
        .split("\r\n")[0]
        .trim();
  ...
  if (checkForUplay && !(await isLauncherInstalled(launcher_location)))
    return false;
  return { registry_res, launcher_location };
}

async function getInstalledGames() {
  const { launcher_location: path, registry_res } =
...
async function parseGameObject(GameID, registry_res) {
  registry_res = registry_res
    .split(window.__TAURI__.os.EOL.repeat(2))
    .find((x) => x.split(window.__TAURI__.os.EOL)[0].endsWith(GameID));
  const Location = registry_res
    ?.split("\r\n")
    .map((x) => x.trim())
  ...
  if (title.match(/\d$/gi) && !title.replaceAll("\\d", "").endsWith(" ")) {
    const numlength = title
      .split("")
      .reverse()
      .join("")
      .match(/\d/gi)[0].length;
    title =
      title.slice(0, title.length - numlength) +
      " " +
      title.slice(title.length - numlength);
  }
  return {
    Executable,
    Location,
