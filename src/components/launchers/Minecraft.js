...
async function getInstalledGames() {
  ...
  if (platform === "win32" || platform === "linux") {
    const e = [
      await getMinecraftLauncher(platform),
      await getLunarClient(platform),
    ].filter((x) => x !== false);
    return e;
  } else {
    return [];
...
async function getMinecraftLauncher(platform) {
  if (platform === "win32") {
    const out = await new shell.Command("cmd", [
      "/C",
      "Reg",
      "query",
    ...
    if (out?.stderr) {
      ...
      const Location = isInstalled
        .split("\r\n")
        .find((x) => x.trim().startsWith("InstallLocation"))
        .split(":")
        .slice(1)
        .join(":")
        .trim();
    ...
    try {
      const output = await new shell.Command(
        "which",
        "minecraft-launcher"
      ).execute();
