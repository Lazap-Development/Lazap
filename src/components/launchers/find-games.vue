...
(async () => {
    ...
    async function loadGames(id) {
        ...
        resolvedGames = resolvedGames.map((game) => {
            ...
            gameBanner.addEventListener('mouseover', () => {
                ...
                const isFavourite = JSON.parse(fs.readFileSync(appDirPath + '/storage/Cache/Games/Data.json').toString()).Games.find(y => y.GameID === game.GameID && y.LauncherName === game.LauncherName && y.Favourite);
    ...
    function setGames(games) {
        ...
        fs.writeFileSync(appDirPath + GAMES_DATA_BASE_PATH, JSON.stringify(games));
    }
    function getGames(GameID, LauncherName) {
        const GAMES_DATA_BASE_PATH = 'storage/Cache/Games/Data.json';

        return GameID ? JSON.parse(fs.readFileSync(appDirPath + GAMES_DATA_BASE_PATH).toString()).Games.find(x => x.GameID === GameID && x.LauncherName === LauncherName) : JSON.parse(fs.readFileSync(appDirPath + GAMES_DATA_BASE_PATH).toString());
    }
